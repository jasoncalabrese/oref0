...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions) {
    ...
    // set ci to a minimum of 5mg/dL/5m when mealCOB >= carbs, and scale to 0 as mealCOB approaches half of carbs
    if (meal_data.mealCOB * 2 > meal_data.carbs) {
        var cob = Math.min(meal_data.mealCOB, meal_data.carbs);
        var minCI = Math.max((10 * (cob / meal_data.carbs)) - 5, 0);
        if (minCI > ci) {
          console.error('Adjusted Carb Impact from', ci, 'to', minCI, 'mg/dL per 5m since COB:', cob, 'g is > 50% of ', meal_data.carbs, 'g');
          ci = minCI;
        }
    }
    aci = 10;
...
//        var maxSafeBasal = Math.min(profile.max_basal, 3 * profile.max_daily_basal, 4 * basal);
        ...
        if (insulinScheduled > insulinReq + 0.1) { // if current temp would deliver >0.1U more than the required insulin, lower the rate
            rT.reason += currenttemp.duration + "m@" + (currenttemp.rate - basal).toFixed(3) + " = " + insulinScheduled.toFixed(3) + " > req " + insulinReq + "+0.1U. Setting temp basal of " + rate + "U/hr";
            return tempBasalFunctions.setTempBasal(rate, 30, profile, rT, currenttemp);
        }
